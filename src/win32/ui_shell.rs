...
use windows::core::{PCWSTR, Result, Error};
...
pub fn get_file_icon(path: &Path) -> Result<(BITMAPINFO, Vec<u8>)> {
  ...
  unsafe {
    ...
    if hicon.is_invalid() {
      ...
      return Err(Error::new(windows::core::HRESULT(0x80004005u32 as i32), "ImageList_GetIcon failed".into()));
    }
    ...
    GetIconInfo(hicon, &mut icon_info).ok().expect("Failed to get icon info");
    ...
    if hbm.is_invalid() {
        return Err(Error::new(windows::core::HRESULT(0x80004005u32 as i32), "Invalid hbmColor".into()));
    }
    ...
    let mut bmih = BITMAPINFOHEADER {
        ...
        biWidth: bmp.bmWidth, biHeight: bmp.bmHeight, biPlanes: 1,
        biBitCount: bmp.bmBitsPixel as u16, biCompression: BI_RGB.0 as u32,
    ...
    if hdc_screen.is_invalid() {
          return Err(Error::new(windows::core::HRESULT(0x80004005u32 as i32), "Failed to get screen DC".into()));
    }
    defer! {{
    ...
    // GetDIBits (1回目)
    let mut bmi_for_size = BITMAPINFO { bmiHeader: bmih, ..Default::default() };
    if GetDIBits(hdc_screen, hbm, 0, bmp.bmHeight as u32, None, &mut bmi_for_size, DIB_RGB_COLORS) == 0 {
        return Err(Error::from_win32());
    }
    ...
    if bmih.biSizeImage == 0 {
        let stride = ((bmih.biWidth.abs() as usize * (bmih.biBitCount as usize / 8) + 3) & !3) as u32;
        bmih.biSizeImage = stride * bmih.biHeight.abs() as u32;
    }
    if bmih.biSizeImage == 0 {
          return Err(Error::new(windows::core::HRESULT(0x80004005u32 as i32), "biSizeImage is zero".into()));
    }
    ...
    // GetDIBits (2回目)
    let mut bmi = BITMAPINFO { bmiHeader: bmih, ..Default::default() };
    let lines = GetDIBits(
        hdc_screen, hbm, 0, bmih.biHeight.abs() as u32,
